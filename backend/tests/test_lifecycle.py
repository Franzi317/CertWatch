...
def test_create_order_starts_pending_approval(db):
    m = _managed_cert(db)
    order = lifecycle.create_order(db, m, "renew", "alice@test.local")
...
def test_create_order_is_idempotent_for_same_open_action(db):
    m = _managed_cert(db)
    first = lifecycle.create_order(db, m, "renew", "alice@test.local")
    second = lifecycle.create_order(db, m, "renew", "alice@test.local")
...
def test_approve_revoke_requires_admin(db):
    m = _managed_cert(db)
    order = lifecycle.create_order(db, m, "revoke", "alice@test.local")
...
def test_transition_reassigns_transitions_list(db):
    m = _managed_cert(db)
    order = lifecycle.create_order(db, m, "renew", "alice@test.local")
    before_id = id(order.transitions)
